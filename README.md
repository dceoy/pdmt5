...
**pdmt5** includes an optional FastAPI-based REST API that exposes MT5 data functionality over HTTP, enabling remote access to market data, account information, and trading history.

### Features

- 🌐 **HTTP Access**: Query MT5 data from any HTTP client
- 📊 **Dual Formats**: JSON and Apache Parquet responses
- 🔒 **Secure**: API key authentication with rate limiting
- 📖 **Auto-documented**: Interactive Swagger UI at `/docs`
- ⚡ **High Performance**: Async operations with FastAPI
- 🎯 **Read-Only**: Secure design excludes trading operations

### Installation

Install pdmt5 with API extras:
...
This installs additional dependencies: FastAPI, uvicorn, pyarrow, and authentication libraries.
...
uvicorn pdmt5.api.main:app --reload
```
...
curl -H "X-API-Key: your-secret-key-here" \
     ...
     -o data.parquet \
     "http://localhost:8000/api/v1/rates/from?symbol=EURUSD&timeframe=60&date_from=2024-01-01T00:00:00Z&count=1000"
```
...
# Get symbols
response = client.get("/api/v1/symbols", params={"group": "*USD*"})
symbols_data = response.json()

# Get historical rates
response = client.get("/api/v1/rates/from", params={
    "symbol": "EURUSD",
    "timeframe": 60,
...
df = pd.DataFrame(rates_data["data"])
```
...
- `GET /api/v1/history/deals` - Get historical deals
...
# Optional MT5 connection (if not using default terminal)
...
For detailed API documentation, see [docs/api/rest-api.md](docs/api/rest-api.md) and [specs/041-mt5-rest-api/quickstart.md](specs/041-mt5-rest-api/quickstart.md).
